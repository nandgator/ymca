...
| Cycle check unimplemented                            | `authorization_edge` accepts a cycle; the DAG is a graph until A2.2's pre-commit recursive CTE exists. R10 said where it runs — `POST /t/{t}/units` — and that endpoint is not built | A2.2           |
...
**Not closed this round, though an earlier draft of this file said it was.**
The cycle-check row above stays open. R10 specified the check — `POST
/t/{t}/units` (A3.7) is to run A2.2's depth-12 recursive CTE before committing
an edge — and specifying is not implementing. That endpoint does not exist
yet, so nothing runs the CTE and nothing inserts an `authorization_edge`
either. The gap is unchanged in substance and merely better described.

Removing the row when the decision was taken rather than when the code landed
would have been the same mistake this document exists to prevent: a record
claiming a guarantee the code does not deliver. It is noted here rather than
silently corrected because the near-miss is the useful part.
